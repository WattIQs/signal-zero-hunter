      ...
      <div className="mx-auto max-w-5xl">
          ...
          <CardContent className="space-y-5 p-4 md:p-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                <Label htmlFor="country" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">País</Label>
                <Select value={selectedCountry} onValueChange={setSelectedCountry} disabled={loadingCountries}>
                  <SelectTrigger id="country" className="bg-background"><SelectValue placeholder="Selecione o país" /></SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedState} onValueChange={setSelectedState} disabled={!selectedCountry || loadingStates || states.length === 0}>
                  <SelectTrigger id="state" className="bg-background"><SelectValue placeholder={states.length === 0 && selectedCountry ? "País sem estados" : "Selecione o estado"} /></SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedCity} onValueChange={setSelectedCity} disabled={!selectedCountry || loadingCities || cities.length === 0}>
                  <SelectTrigger id="city" className="bg-background"><SelectValue placeholder="Selecione a cidade" /></SelectTrigger>
            ...
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-4 rounded-lg border border-border bg-background/60 p-3">
                <div>
                  <Label htmlFor="only-contactable" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Só quem eu consigo contatar</Label>
                  <p className="mt-1 text-xs text-muted-foreground/80">Mostra apenas leads com WhatsApp/telefone ou Instagram para abordagem direta.</p>
                </div>
                <Switch id="only-contactable" checked={onlyContactable} onCheckedChange={setOnlyContactable} />
              </div>
              <div className="flex items-start justify-between gap-4 rounded-lg border border-border bg-background/60 p-3">
                <div>
                  <Label htmlFor="only-low" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Só quem tem presença fraca</Label>
                  <p className="mt-1 text-xs text-muted-foreground/80">Esconde quem já está bem digitalizado (site + redes). Use "Ver avaliações" no card para conferir as estrelas no Google em segundos.</p>
                </div>
        ...
        {scanning && (
          <div className="mb-8 flex flex-col items-center justify-center py-8">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Geocodificando e varrendo o território...</p>
          </div>
        )}

        {results.length > 0 && !scanning && (
          <div className="mb-6 grid grid-cols-3 gap-3">
            <Card className="border-signal-zero/30 bg-signal-zero/10"><CardContent className="p-3 text-center"><p className="text-2xl font-bold text-signal-zero">{counts.zero}</p><p className="text-[10px] font-medium uppercase tracking-wider text-signal-zero/80">Sinal Zero</p></CardContent></Card>
            <Card className="border-signal-weak/30 bg-signal-weak/10"><CardContent className="p-3 text-center"><p className="text-2xl font-bold text-signal-weak">{counts.weak}</p><p className="text-[10px] font-medium uppercase tracking-wider text-signal-weak/80">Sinal Fraco</p></CardContent></Card>
            <Card className="border-cyan/30 bg-cyan/10"><CardContent className="p-3 text-center"><p className="text-2xl font-bold text-cyan">{counts.contactable}</p><p className="text-[10px] font-medium uppercase tracking-wider text-cyan/80">Contatáveis</p></CardContent></Card>
          </div>
        )}
        ...
        {results.length > 0 && !scanning && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-foreground">Resultados ({sortedResults.length})</h2>
              <span className="text-xs text-muted-foreground">Cidade inteira · contatáveis primeiro</span>
            </div>
        ...
        {!scanning && results.length === 0 && !error && (
          <div className="mt-12 text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Selecione uma cidade e clique em "Escanear área" para encontrar leads.</p>
          </div>
        )}
      </div>
    </div>
