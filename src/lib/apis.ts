...
export async function geocodeCity(country: string, state: string | null, city: string): Promise<GeoPoint> {
  ...
  return fetchWithRetry(async () => {
    ...
    return {
      ...
      boundingBox: bb ? { south: Number.parseFloat(bb[0]), north: Number.parseFloat(bb[1]), west: Number.parseFloat(bb[2]), east: Number.parseFloat(bb[3]) } : null,
    };
...
function buildOverpassQuery(area: { south: number; north: number; west: number; east: number }, categories: CategoryKey[]): string {
  ...
  for (const key of categories) {
    ...
    for (const value of category.osmValue.split(";")) {
  ...
  if (filters.size === 0) throw new Error("Selecione ao menos uma categoria válida.");
...
export async function searchOverpass(area: { south: number; north: number; west: number; east: number }, categories: CategoryKey[]): Promise<{
  elements: {
    type: string;
    id: number;
    lat?: number;
